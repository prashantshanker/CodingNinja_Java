...
public class SecondLargestElement {
	...
	public static void secondLargestUtil(TreeNode<Integer> root) {
		if (root == null) {
			return;
		}
		if (first == null) {
			...
			second = first;
			first = root;
		} else if ((second == null) && (root.data != first.data )) {
			second = root;
		}
		for (int i = 0; i < root.children.size(); i++) {
			secondLargestUtil(root.children.get(i));
		}
		return;
	}
